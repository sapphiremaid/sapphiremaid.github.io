...
function factCard(label,key){
  ...
  return '<div class="metric '+esc(f.state)+'"><div class="label">'+esc(label)+'</div><div class="value">'+esc(shown)+'</div><div class="state">'+esc(String(f.state||"unknown").toUpperCase())+(f.kind==="derived"?" · DERIVED":"")+'</div><div class="source" title="'+esc(f.source)+'">'+esc(f.source||"source unavailable")+'</div></div>';
}
...
function viewMeta(v){
  var map={
    overview:["House","Systems, agents, projects, and recent work."],
    agents:["Agents","Durable House agents, their bindings, schedules, and latest observed runs."],
    projects:["Projects","Project and operational-domain status from their current sources."],
    schedules:["Schedules","Durable wakes known to Mecha Runtime. Disabled is not the same thing as absent."],
    activity:["Activity","Recent runs. Historical runs are not treated as current activity."],
    proof:["Sources","Where the current data comes from, what is stale, and what is unavailable."]
  };
  return map[v];
}
function render(){
...
function renderOverview(){
  ...
  }).join(""):'<div class="empty">No current attention items.</div>';
  ...
  }).join(""):'<div class="empty">No known coverage gaps.</div>';
  ...
  return '<div class="truth-note"><div><strong>Freshness:</strong> current values require a recent source observation. Missing data stays unknown.</div><code>local · read-only</code></div>'+
    ...
    '<section class="panel"><div class="panel-head"><h2>Coverage</h2><span>'+coverage.length+' gaps</span></div><div class="panel-body">'+cov+'</div></section></div>'+
    '<section class="panel" style="margin-top:14px"><div class="panel-head"><h2>System checks</h2><span>green = current evidence supports the named check</span></div><div class="claims">'+cs+'</div></section>';
}
...
function renderProjects(){
  ...
  return '<div class="truth-note"><div><strong>Status handling:</strong> Unknown stays Unknown; the dashboard does not infer inactive, healthy, or abandoned.</div><code>current catalog</code></div>'+
...
async function load(){
  ...
  try{
    ...
    q("#freshness").textContent="updated "+ageText(next.generated_at)+" · "+((next.runtime_health&&next.runtime_health.freshness)||"runtime status unknown");
    setBanner(null); render();
  }catch(err){
    data=null; lastError=String((err&&err.message)||err); setLink("bad","LOCAL LINK DOWN");
    q("#freshness").textContent="current data unavailable";
