...
export function createIslandSurfaceSpatialIndex(islands, { cellSize = DEFAULT_CELL_SIZE } = {}) {
  ...
  // Source traversal is already immutable world order, so each cell is ordered once
  // during construction. Freeze the finished bucket and reuse it for every terrain
  // sample instead of allocating/sorting/mapping a fresh candidate list per query.
